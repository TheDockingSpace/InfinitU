...
case class Integer2DObjectAddress(val firstValue: Integer = 0, val secondValue: Integer = 0, val dimensions: List[Dimension[_]] = List(IntegerDimension("x"), IntegerDimension("y"))) extends ObjectAddress {

  override val values = List(IntegerValue(firstValue), IntegerValue(firstValue))

  override def withValues[A <: ObjectAddress](values: List[DimensionValue[_]]): A = Integer2DObjectAddress(values(0).asInstanceOf[Integer], values(1).asInstanceOf[Integer], dimensions).asInstanceOf[A]

}

class Integer2DUniverse[V <: Comparable[_]](override val name: String = "Integer2DUniverse", val firstDimensionName: String = "x", val secondDimensionName: String = "y", override val objects: Map[Integer2DObjectAddress, _ <: V] = Map())
  ...
  override def withObjects(objects: Map[Integer2DObjectAddress, V]) = {
    new Integer2DUniverse(name, firstDimensionName, secondDimensionName, objects)
  }

}

case class Integer3DObjectAddress(val firstValue: Integer = 0, val secondValue: Integer = 0, val thirdValue: Integer = 0, val dimensions: List[Dimension[_]] = List(IntegerDimension("x"), IntegerDimension("y"), IntegerDimension("z"))) extends ObjectAddress {

  override val values = List(IntegerValue(firstValue), IntegerValue(firstValue), IntegerValue(thirdValue))

  override def withValues[A <: ObjectAddress](values: List[DimensionValue[_]]): A = Integer3DObjectAddress(values(0).asInstanceOf[Integer], values(1).asInstanceOf[Integer], values(2).asInstanceOf[Integer], dimensions).asInstanceOf[A]

}

class Integer3DUniverse[V <: Comparable[_]](override val name: String = "Integer3DUniverse", val firstDimensionName: String = "x", val secondDimensionName: String = "y", val thirdDimensionName: String = "z", override val objects: Map[Integer3DObjectAddress, V] = Map())
  ...
  override def withObjects(objects: Map[Integer3DObjectAddress, V]) = {
    new Integer3DUniverse(name, firstDimensionName, secondDimensionName, thirdDimensionName, objects)
  }
