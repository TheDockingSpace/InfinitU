...
sealed trait Color extends Ordered[Color] {
  ...
  def compare(that: Color): Int =
    Color.values.indexOf(this) compare Color.values.indexOf(that)
...
case object Color {
  ...
  val oppositeTo: Map[Color, Color] = Map(First -> Sixth,
                                          Second -> Fourth,
                                          Third  -> Fifth,
                                          Fourth -> Second,
                                          Fifth  -> Third,
  ...
  val adjacentTo: Map[Color, List[Color]] = values.map { color =>
    color -> values.filterNot { c =>
      c == color || c.opposite == color
    }.toList
  }.toMap
...
sealed trait Piece extends Ordered[Piece] {
  def compare(that: Piece): Int =
    Pieces.values.indexOf(this) compare Pieces.values.indexOf(that)
}
  ...
  val values: List[Piece] = {
    val pieces = (for (front <- Color.values) yield {
      ...
      (for (top <- front.adjacent) yield {
        ...
        val corners: List[Piece] =
          for (side <- front.adjacent.filterNot(c => c == top || c == front))
            yield CornerPiece(front, top, side)
        edge :: corners
      }).flatten
    }).flatten
    pieces
...
case class CubePuzzle(
    override val objects: Map[Integer3DObjectAddress, Piece] = Map())
    extends Integer3DUniverse[Piece](objects = objects)
