...
object BooleanGenesPopulation {
  def apply(populationSize: Integer, chromosomeSize: Integer): BooleanGenesPopulation = {
    BooleanGenesPopulation(BooleanGenesIndividuals(chromosomes = (for (chromosome <- 0 until populationSize) yield (Integer1DObjectAddress(chromosome) -> BooleanGenesChromosome(chromosomeSize))).toMap))
  }
}
...
case class BooleanGenesIndividuals(
  ...
  val chromosomes: Map[Integer1DObjectAddress, BooleanGenesChromosome] = Map.empty)
    ...
    extends Integer1DUniverse[BooleanGenesChromosome](objects = chromosomes)
