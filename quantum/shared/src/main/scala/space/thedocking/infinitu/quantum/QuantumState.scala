...
trait Collapser[V] {
  ...
  def collapse(superposition: Superposition[V],
               times: Int): Map[CollapsedValue[V], Int]

}

trait Superposition[V] extends QuantumState[V] { self: Finiteness[V] =>

  override val isSuperposition = true

  def collapse(collapser: Collapser[V]): CollapsedValue[V] =
    collapser.collapse(this)

  def collapse(collapser: Collapser[V],
               times: Int): Map[CollapsedValue[V], Int] =
...
class RandomFiniteSuperpositionCollapser[V <: DimensionValue[_]]
    extends Collapser[V] {
  ...
  override def collapse(superposition: Superposition[V],
                        times: Int): Map[CollapsedValue[V], Int] = {
...
trait CollapsedBooleanValue extends CollapsedValue[BooleanValue] {
  override val superposition: Superposition[BooleanValue] =
...
object Plotter {
  ...
  implicit object ComparableOrdering extends Ordering[Comparable[_]] {
    def compare(c1: Comparable[_], c2: Comparable[_]): Int =
      c1.compareTo(c2.asInstanceOf)
  }

  def plot[V <: DimensionValue[_ <: Comparable[_]]](
      values: Map[CollapsedValue[V], Int]): Seq[String] = {
    ...
    for ((k, v) <- sorted)
      yield
        s"${k.padTo(maxLabel, " ").mkString("")} ${"=" * (maxBar * v / maxCount)}"
  }
